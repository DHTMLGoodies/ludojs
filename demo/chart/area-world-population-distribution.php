    ...
    var dataSource = new ludo.chart.DataSource({
        ...
        textOf:function(record, caller){
            if(caller.type == 'chart.Tooltip'){
                return '<p><b>{parent.region} - {record.year}</b><br>' +
                    'Population: {record.population} '  + 'million (' + (Math.round(record.__indexFraction * 1000)/10) + '%)' +
                    '<br>World Population: {record.__indexSum} million</p>';
            }
