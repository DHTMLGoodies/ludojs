...
<script type="text/javascript" class="source-code">
    new ludo.Window({
        ...
        form:{
            url:'data-source/progress-bar.php'
        },
        layout:'rows',
        children:[
            { html:'Click on Submit will demonstrate the progress bar '},
            { type:'progress.Bar', url:'data-source/progress-bar.php', pollFrequence:.5, listeners:{
                finish:function () {
                    var cmp = ludo.get('progress-bar-window')
                    cmp.dispose.delay(1000, cmp);
                }
            } },
            { type:'progress.Text', url:'data-source/progress-bar.php', tpl:'Current Status: {text}' }
        ],
        listeners:{
            'submit':function () {
