TestCase("MenuTest", {
	...
	setUp:function () {
		if (ludo.ViewWithFixedCoordinates === undefined) {
			ludo.ViewWithFixedCoordinates = new Class({
	...
	"test should set orientation correctly for menu.Item custom classes":function () {
		// given
		var c = new ludo.menu.Context({
			children:[
				{
		...
		assertEquals('vertical', c.child['a'].child['b'].orientation);
	},
